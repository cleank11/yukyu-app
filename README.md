...
if is_admin:
    with tab_approve:
        st.header("💮 有休の承認・却下手続き")
        if not df_req.empty and ("ステータス" in df_req.columns):
            pending_reqs = df_req[df_req["ステータス"] == "承認待ち"]

            if pending_reqs.empty:
                st.success("現在、承認待ちの有休申請はありません。")
            else:
                for index, row in pending_reqs.iterrows():
                    ...
                    with st.container():
                        st.markdown(
                            f"**【申請ID: {req_id}】 {emp_name} さん** から **{req_days}日** の有休申請"
                        )
                        col_ok, col_ng = st.columns(2)

                        if col_ok.button(
                            f"👍 承認する（ID:{req_id}）", key=f"ok_{req_id}"
                        ):
                            df_req.at[index, "ステータス"] = "承認済み"
                            save_all(df, df_req)
                            st.success(f"ID:{req_id} の有休を承認しました！")
                            st.rerun()

                        if col_ng.button(
                            f"👎 却下する（ID:{req_id}）", key=f"ng_{req_id}"
                        ):
                            df_req.at[index, "ステータス"] = "却下"
                            save_all(df, df_req)
                            st.warning(f"ID:{req_id} の有休申請を却下しました。")
                            st.rerun()
                    st.markdown("---")
        else:
            st.success("現在、承認待ちの有休申請はありません。")
    ...
    with tab2:
        st.header("✍️ 年度別出勤データの確認")
        st.write(
            "スプレッドシート側で編集した内容がここにリアルタイムで表示されます。"
        )

    with tab3:
        st.header("👤 新しいパート社員の登録")
        st.write(
            "新しい社員は、お手元のGoogleスプレッドシート（yukyu_master）の行に直接追加するだけで自動的にここに増えます！"
        )

    with tab4:
        st.header("⚙️ 登録データ一覧（Googleスプレッドシート同期データ）")
        st.subheader("【社員マスタ（ネット上のデータ）】")
        st.dataframe(df)
        st.subheader("【申請履歴（ネット上のデータ）】")
        st.dataframe(df_req)
